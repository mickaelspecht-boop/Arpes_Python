...
# Heavy, recomputable, display-only arrays that must NEVER be persisted in the
# session JSON. ``I_smoothed`` is the full smoothed band-map image (~20 MB per
# fit, 980×1064 floats); storing it bloated real sessions past 300 MB and made
# every save()/load multi-second (the root cause of the load/reload/tab-switch
# lag). It is regenerated by the MDC fitters and is read nowhere at runtime.
EPHEMERAL_FIT_RESULT_KEYS = ("I_smoothed",)


def strip_ephemeral_fit_result(fr, *, in_place: bool = False):
    """Return ``fr`` without the heavy ephemeral display arrays.

    Shallow: the dropped values are never copied. ``in_place`` mutates and
    returns the same dict (used at load, to free RAM and clean legacy bloated
    sessions); otherwise a shallow copy without the keys is returned (used at
    save so the live in-memory dict keeps its arrays for the current display)."""
    if not isinstance(fr, dict):
        return fr
    if in_place:
        for k in EPHEMERAL_FIT_RESULT_KEYS:
            fr.pop(k, None)
        return fr
    if not any(k in fr for k in EPHEMERAL_FIT_RESULT_KEYS):
        return fr
    return {k: v for k, v in fr.items() if k not in EPHEMERAL_FIT_RESULT_KEYS}


def _entry_to_payload(entry) -> dict:
    """Serialize one FileEntry, dropping ephemeral heavy fit-result arrays
    WITHOUT deep-copying them. ``asdict`` deep-copies every value, so the heavy
    arrays are swapped out of the live entry for the duration of the call and
    restored in ``finally`` (save runs single-threaded on the UI path)."""
    fr = entry.fit_result
    zones = entry.fit_zones
    swap_fr = isinstance(fr, dict) and any(k in fr for k in EPHEMERAL_FIT_RESULT_KEYS)
    swap_zones = bool(zones) and any(
        isinstance(z, dict) and isinstance(z.get("fit_result"), dict)
        and any(k in z["fit_result"] for k in EPHEMERAL_FIT_RESULT_KEYS)
        for z in zones
    )
    if swap_fr:
        entry.fit_result = strip_ephemeral_fit_result(fr)
    if swap_zones:
        entry.fit_zones = [
            {**z, "fit_result": strip_ephemeral_fit_result(z.get("fit_result"))}
            if isinstance(z, dict) else z
            for z in zones
        ]
    try:
        return _to_serial(asdict(entry))
    finally:
        if swap_fr:
            entry.fit_result = fr
        if swap_zones:
            entry.fit_zones = zones


def normalize_groups(raw) -> list[dict]:
    """Validate ``Session.groups``: ordered, named, colour-tagged file groups.

    Each group is ``{"name": str, "color_idx": int, "members": list[str]}``.
    Drops malformed entries and duplicate names (case-insensitive), de-dupes
    members while preserving order. Membership is meant to be exclusive (one
    group per file); that invariant is enforced by the editing helpers in
    ``analysis/result_groups.py``, not here, so a hand-edited JSON never loses
    data silently — it is only normalized."""
    out: list[dict] = []
    seen: set[str] = set()
    for g in raw or []:
        ...
        for m in g.get("members", []) or []:
            ms = str(m)
            if ms and ms not in mseen:
                mseen.add(ms)
                mem.append(ms)
        out.append({
            "name": name,
            "color_idx": int(g.get("color_idx", 0) or 0),
            "members": mem,
        })
    return out
...
class Session:
    ...
    # v5 -> v6: added Session.groups (named, colour-tagged Results groups for
    # organising fitted files). v5->v6 migration = absent field -> empty list.
    VERSION = 6
    ...
    def __init__(self, folder: Path | None = None, work_func: float = 0.0):
        ...
        # Results panel organisation: ordered, named, colour-tagged groups of
        # fitted files {"name", "color_idx", "members"}. Exclusive membership
        # (one group per file). Auto-grouping by metadata is derived on the fly
        # and never stored here. See analysis/result_groups.py.
        self.groups: list[dict] = []
        # Version of the payload that was actually loaded (None for a new session).
        self.loaded_version: Optional[int] = None
        # UI may install a debounce trampoline so rapid save() calls coalesce
        # into a single disk write (see set_save_scheduler). None = write
        # immediately (headless callers, tests).
    ...
    def reset(self, *, keep_folder: bool = True) -> None:
        ...
        self._save_scheduler = scheduler  # survive folder reloads
    ...
    def to_payload(self) -> dict:
        ...
        return {
            ...
            "groups": _to_serial(self.groups),
            "files": {
                name: _entry_to_payload(entry)
    ...
    def save(self):
        ...
        When a save scheduler is installed (the UI), this only *requests* a
        debounced save so rapid edits and the multi-save load path don't each
        block on a full JSON write — the actual write fires once after a quiet
        period. Headless callers (no scheduler) write immediately. Critical
        paths call ``flush_save()``."""
    ...
    def flush_save(self) -> None:
        """Write immediately, bypassing the debounce (UI close, export, and the
        debounce-timer timeout)."""
        self._save_now()

    def set_save_scheduler(self, callback) -> None:
        """Install a debounce trampoline (UI layer). ``callback`` is invoked on
        every save request and should (re)start a timer that later calls
        ``flush_save()``. Pass None to restore immediate writes."""
        self._save_scheduler = callback
    ...
    def load_from_payload(self, raw: dict) -> None:
        ...
        # v5->v6: absent field -> empty list (no Results groups).
        self.groups = normalize_groups(raw.get("groups", []))
        self.files = {}
        for name, edict in raw.get("files", {}).items():
            ...
            # Drop any persisted heavy display arrays (legacy bloated sessions):
            # frees RAM now and the next save() rewrites the file small.
            strip_ephemeral_fit_result(entry.fit_result, in_place=True)
            for _z in entry.fit_zones:
                if isinstance(_z, dict):
                    migrate_fit_result_to_hwhm(_z.get("fit_result"))
                    strip_ephemeral_fit_result(_z.get("fit_result"), in_place=True)
